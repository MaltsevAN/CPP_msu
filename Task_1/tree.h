...
namespace bintree {
    ...
    struct TNode: std::enable_shared_from_this<TNode<T>> {
        ...
        using TNodeWeakConstPtr = std::weak_ptr<const TNode<T>>;
    ...
    private:
        ...
        TNode(T v, TNode* left, TNode* right)
                : value(v)
                , left(left ? left->shared_from_this() : nullptr)
                , right(right ? right->shared_from_this() : nullptr)
        {
        }
